...
public class HexMapGenerator : MonoBehaviour
{
    ...
    [Range(0, 100)]
    public int erosionPersentage = 50;
    [Range(0f, 1f)]
    ...
    //    public Base(int MainHole)
    //    {
    //        this.MainHole = MainHole;
    //    }
    //}
    //static HexTownBase.Base[] bases =
    //{
    //    new HexTownBase.Base(1), new HexTownBase.Base(2), new HexTownBase.Base(3)
    //};
    ...
    static Biome[] biomes =
    {
        ...
        new Biome(0, 7), new Biome(2, 0), new Biome(2, 5), new Biome(2, 6),
        new Biome(0, 0), new Biome(1, 0), new Biome(1, 1), new Biome(1, 2),
        new Biome(0, 8), new Biome(1, 1), new Biome(1, 2), new Biome(1, 3)
    };
    ...
    public void GenerateMap(int x, int z, bool wrapping)
    {
        ...
        if (!useFixedSeed)
        {
            ...
            seed ^= (int)Time.unscaledDeltaTime;
            seed &= int.MaxValue;
        }
    ...
    HexCell randomSpawnCell ()
    {
        List<HexCell> cells = new List<HexCell>();
        HexCell spawnCell;
        for (int i = 0; i < cellCount; i++)
        {
            spawnCell = grid.GetCell(i);
            if (spawnCell.isValidPosition())
            {
                for (HexDirection d = HexDirection.NE; d <= HexDirection.NW; d++)
                {
                    if (spawnCell.GetElevationDifference(d) < 1 && spawnCell.GetNeighbor(d).isValidPosition())
                    {
                        cells.Add(spawnCell);
                    }
                }
        ...
        for (HexDirection d = HexDirection.NE; d <= HexDirection.NW; d++)
        {
            spawnCell.GetNeighbor(d);
        }
        cells.Clear();
        return spawnCell;
    }

    public HexCell getRandomNeighbor(HexCell cell)
    {
        List<HexCell> cells = new List<HexCell>();
        HexCell neighbor;
        for (HexDirection d = HexDirection.NE; d <= HexDirection.NW; d++)
    ...
    void SpawnUnit()
    {
        ...
        HexCell spawnCell = randomSpawnCell();
        HexCell neighbor;
        HexMapCamera.ValidPosition.x = (int)spawnCell.transform.position.x;
        HexMapCamera.ValidPosition.z = (int)spawnCell.transform.position.z;
        
        HexUnit.unitPrefab = grid.Races[HexRaces.RaceIndex].townHole;
        if (spawnCell.PlantLevel > 0)
        {
            spawnCell.PlantLevel = 0;
        }
        grid.AddUnit(
            Instantiate(HexUnit.unitPrefab), spawnCell, Random.Range(0f, 360f), true);

        neighbor = getRandomNeighbor(spawnCell);

        HexUnit.unitPrefab = grid.Races[HexRaces.RaceIndex].buildUnit;
        grid.AddUnit(
            Instantiate(HexUnit.unitPrefab), neighbor, Random.Range(0f, 360f), true);

        neighbor = getRandomNeighbor(spawnCell);
        HexUnit.unitPrefab = grid.Races[HexRaces.RaceIndex].meleeUnit;

        grid.AddUnit(
            Instantiate(HexUnit.unitPrefab), neighbor, Random.Range(0f, 360f), true);
        //        HexTownBase.Base st = bases[Random.Range(0, 3)];
        //        spaw.SpecialIndex = st.MainHole;
    ...
    void SetTerrainType()
    {
        ...
        for (int i = 0; i < cellCount; i++)
        {
            ...
            if (!cell.IsUnderwater)
            {
                ...
                else if (cellBiome.plant < 3 && cell.HasRiver && cellBiome.terrain != 3)
                {
                    cellBiome.plant += 1;
                }
            ...
            else
            {
                ...
                if (cell.Elevation == waterLevel - 1)
                {
                    ...
                    for(
                        ...
                        d <= HexDirection.NW; d++)
                    {
                        HexCell neihgbor = cell.GetNeighbor(d);
                        if (!neihgbor)
                        {
                            continue;
                        }
                        int delta = neihgbor.Elevation - cell.WaterLevel;
                        if (delta == 0)
                        {
                            slopes += 1;
                        }
    ...
    void ErodeLand()
    {
        ...
        int targetErodibleCount =
            (int)(erodibleCells.Count * (100 - erosionPersentage) * 0.01f);
